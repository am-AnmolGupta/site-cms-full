...
const ViewProfiles = () => {
    ...
    const getProfiles = async () => {
      try {
        ...
        const response = await fetch(
          `${url}/admin/profiles?page=${paginationModel.page + 1}&limit=${paginationModel.pageSize}`,
          {
            method: "GET",
            headers: {
  ...
  const handleEdit = (item) => {
    navigate(`/profiles/${item._id}/edit`, { state: { itemData: item } });
  };
    ...
    <>
      ...
      <CCard className="mb-4">
        <CCardHeader className="d-flex justify-content-between align-items-center">
          <h3 className="m-0">Profiles Dashboard</h3>
          <CButton
            color="primary"
            variant="outline"
